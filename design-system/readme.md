...
- **Type.** Three roles, three faces: silkscreen labels = Barlow Condensed 600 uppercase, 9–18px, `letter-spacing .08em`; LCD = VT323 pixel mono, uppercase, `line-height 1`, `white-space: pre`, on a fixed character grid; body = Barlow 400/500 14px only outside the machine. Wordmark = Barlow Condensed 700. *(All Google-Fonts substitutes — see caveats.)*
