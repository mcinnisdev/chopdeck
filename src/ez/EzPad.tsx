...
export interface EzPadProps { index: number; name: string; hotkey: string; lit: boolean; onDown(vel: number): void; onUp(): void; /** empty pads accept a press too (choosing a pad to put a sound on) */ armed?: boolean }

export function EzPad({ index, name, hotkey, lit, onDown, onUp, armed = false }: EzPadProps) {
  const down = useRef(false);
  const has = name.length > 0 || armed;
  const style: CSSProperties = {
    aspectRatio: '1', width: '100%', border: 'var(--stroke-w) solid var(--ink)', borderRadius: 'var(--radius-pad)',
    background: lit ? 'var(--red-lit, #ff8a72)' : name ? 'var(--red)' : 'var(--cream-3)',
  ...
  return (
    <button type="button" aria-label={`Pad ${index + 1}${name ? `: ${name}` : ', empty'}`} aria-pressed={lit} style={style}
      ...
      <span style={{ position: 'absolute', left: 10, bottom: 8, right: 10, fontSize: 'clamp(11px, 1.6vw, 16px)', lineHeight: 1.05, textAlign: 'left', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{name || 'empty'}</span>
