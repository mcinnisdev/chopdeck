...
async function doLoad(c: Ctx) {
  ...
  if (kind === '.CHOPDECK' || kind === '.ZIP') { const p = decodeProject(bytes); if (!p) { c.fw.message('NOT A PROJECT'); return; } c.fw.confirm({ title: 'Load Project', lines: [`File:${f.name}`, '', 'This will replace EVERYTHING in memory!'], doItLabel: 'LOAD', doIt: () => { c.fw.loadProject(p); if (diskState.device === 'IMPORT') dropImport(c, f.name); c.fw.message('PROJECT LOADED'); } }); return; }
